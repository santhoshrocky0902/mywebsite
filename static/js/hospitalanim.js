...
if ($('.achieve_sec').length > 0) {
    ...
    var studies1 = studies.fromTo('.studies_sec .title_line', 0.8, { y: 30, opacity: 0 }, { y: 0, opacity: 1 })
        .staggerFromTo('.study_ug .course-box', 0.3, { y: 30, opacity: 0 }, { y: 0, opacity: 1 }, 0.05)
        .staggerFromTo('.study_pg .course-box', 0.3, { y: 30, opacity: 0 }, { y: 0, opacity: 1 }, 0.05);
    new ScrollMagic.Scene({ triggerElement: '.studies_sec', reverse: false, triggerHook: 'onEnter', offset: 0 }).setTween(studies1).addTo(controller);



    /* News & Events */
    var news2 = new TimelineLite();
    var news3 = news2.fromTo('.col-sm-12 .title_line', 0.8, { y: 30, opacity: 0 }, { y: 0, opacity: 1 });
    new ScrollMagic.Scene({ triggerElement: '.news_events', reverse: false, triggerHook: 'onEnter', offset: 250 }).setTween(news2).addTo(controller);

    var news = new TimelineLite();
    var news1 = news.staggerFromTo('.news_events_left >*', 0.8, { y: 30, opacity: 0 }, { y: 0, opacity: 1 }, 0.2);
    new ScrollMagic.Scene({ triggerElement: '.news_events_left', reverse: false, triggerHook: 'onEnter', offset: 250 }).setTween(news1).addTo(controller);

    var news = new TimelineLite();
    var news1 = news.staggerFromTo('.news_events_rgt >*', 0.8, { y: 30, opacity: 0 }, { y: 0, opacity: 1 }, 0.2);
    new ScrollMagic.Scene({ triggerElement: '.news_events_left', reverse: false, triggerHook: 'onEnter', offset: 250 }).setTween(news1).addTo(controller);
